...
class TrafficLightNode:
    def __init__(self, node_name, node_id):
        ...
        # Allow immediate reuse of the port if possible (to avoid TIME_WAIT issues)
        try:
        ...
        self.discovery = discovery.Discovery(self.node_id, on_peer_discovered=self.heartbeat.update_peer)
        ...
        # Set up heartbeat callbacks for node join/leave
        self.heartbeat.on_node_joined = self._handle_node_joined
        # On node leave, trigger election's peer-failed handler (which handles leader failure)
    ...
    def start(self):
        ...
        if self._running:
            return
        self._running = True
        # Start discovery (multicast send/receive threads)
        self.discovery.start()
        # Start heartbeat monitor thread
        self.heartbeat.start()
        # Start sequencer thread (resend monitor)
        self.sequencer.start()
        # Start thread to receive unicast messages (election, phase updates, etc.)
        recv_thread = threading.Thread(target=self._unicast_receive_loop, daemon=True)
        recv_thread.start()
        # Give discovery a moment to collect initial peers
        time.sleep(1)
    ...
    def stop(self):
        ...
        # Stop sequencer service
        try:
            self.sequencer.stop()
        except Exception as e:
            logger.error(f"Error stopping sequencer: {e}")
        # If this node was leader and running the phase loop, signal it to stop
        if self._phase_thread and self._phase_thread.is_alive():
            logger.info("Waiting for phase loop to terminate...")
            # The loop will check self._running or leadership and exit
        # Close the unicast socket to break the receive loop
        try:
    ...
    def _unicast_receive_loop(self):
        ...
        while self._running:
            ...
            if msg_type == message.TYPE_ELECTION:  # 'ELECTION'
            ...
            elif msg_type == message.TYPE_COORDINATOR:  # 'COORDINATOR'
                self.election.handle_coordinator_message(msg)
            elif msg_type == message.TYPE_PHASE_UPDATE:  # 'PHASE_UPDATE'
            ...
            elif msg_type == message.TYPE_ACK:  # 'ACK'
                ...
                # Heartbeat messages handling (if implemented separately)
                pass
            else:
    ...
    def _handle_leader_elected(self, leader_id, leader_name):
        ...
        if leader_id == self.node_id:
            ...
            # Synchronize sequencer sequence number with last known sequence from previous leader
            last_seq = getattr(self.sequencer, '_last_received_seq', 0)
            self.sequencer.sync_sequence_num(last_seq)
            # Start traffic light phase coordination loop in a separate thread
            ...
            # If this node was previously leader, the phase loop will stop on its own
            # Reset sequencer state (clear any pending leader state)
    ...
    def _phase_update_loop(self):
        ...
        while self._running and self.election.is_leader:
            ...
            for node_id in all_nodes:
                ...
                if current_green_id == self.node_id:
